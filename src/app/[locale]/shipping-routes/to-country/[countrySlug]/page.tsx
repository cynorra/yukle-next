...
import { getDestinationCountryHub, normalizeCountryName } from '@/lib/laneRoutes';
...
const LABELS: Record<string, {
  ...
  tr: {
    ...
    desc: (c) => `${c} varışlı aktif yük ilanlarını görüntüleyin. Bu ülkeye yük taşıyan nakliyecilerle doğrudan iletişime geçin.`,
    ...
    noLoads: 'Bu ülkeye şu anda aktif ilan bulunmuyor. Yeni ilanlar için tekrar kontrol edin veya tüm ilanları inceleyin.',
    postCta: 'Bu Ülkeye İlan Ver',
  ...
  en: {
    ...
    desc: (c) => `Browse active freight loads heading to ${c}. Connect directly with shippers posting loads bound for this country.`,
    ...
    noLoads: 'No active loads to this country right now. Check back soon, or browse all current listings.',
    postCta: 'Post a Load To This Country',
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (rawLocale !== 'tr' && rawLocale !== 'en') {
    return {
      alternates: { canonical: `${SITE_URL}/en/shipping-routes/to-country/${countrySlug}` },
    };
  }
  ...
  const hub = await getDestinationCountryHub(countrySlug);
  ...
  // instead of self-referencing (same fix as the origin country page).
  ...
  const languages = translatedLocales.reduce((acc, code) => {
    acc[code] = `${SITE_URL}/${code}/shipping-routes/to-country/${countrySlug}`;
    return acc;
  }, {} as Record<string, string>);
  languages['x-default'] = `${SITE_URL}/en/shipping-routes/to-country/${countrySlug}`;

  return {
    title,
    description,
    alternates: {
      canonical: `${SITE_URL}/${canonicalLocale}/shipping-routes/to-country/${countrySlug}`,
      languages,
    },
    openGraph: {
      title: `${title} | Loadly`,
      description,
      url: `${SITE_URL}/${canonicalLocale}/shipping-routes/to-country/${countrySlug}`,
    },
  };
}
...
export default async function DestinationCountryHubPage({ params }: Props) {
  const { locale: rawLocale, countrySlug } = await params;

  if (rawLocale !== 'tr' && rawLocale !== 'en') {
    redirect(`/en/shipping-routes/to-country/${countrySlug}`);
  }
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: t.activeLoads, item: `${SITE_URL}/${locale}/marketplace` },
      {
        ...
        item: `${SITE_URL}/${locale}/shipping-routes/to-country/${countrySlug}`,
      },
    ],
  };
                ...
                >
                  ...
                  <div className="text-xs text-muted flex flex-col gap-1.5">
                    <span className="flex items-center gap-1.5 font-semibold">
                      <MapPin size={13} /> {load.origin_city}, {normalizeCountryName(load.origin_country || '')} → {load.destination_city}, {hub.country}
                    </span>
                    {load.weight_ton && (
