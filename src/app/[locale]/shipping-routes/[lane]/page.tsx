...
const LABELS: Record<string, {
  ...
  browseCta: string;
  faqTitle: string;
  faq: (o: string, d: string, count: number) => { q: string; a: string }[];
}> = {
  tr: {
    ...
    faq: (o, d, count) => [
      {
        q: `${o} - ${d} güzergahında kaç aktif yük ilanı var?`,
        a: count > 0
          ? `Şu anda bu güzergahta ${count} aktif yük ilanı bulunuyor. Bu sayı ilanlar tamamlandıkça veya yenileri eklendikçe değişir.`
          : `Bu güzergahta şu anda aktif ilan bulunmuyor, ancak yeni ilanlar eklendikçe sayfa otomatik güncellenir.`,
      },
      {
        q: `${o} - ${d} güzergahında nasıl nakliyeci bulurum?`,
  ...
  en: {
    ...
    faq: (o, d, count) => [
      {
        q: `How many active freight loads are there from ${o} to ${d}?`,
        a: count > 0
          ? `There are currently ${count} active freight loads on this lane. This number changes as loads are completed or new ones are posted.`
          : `There are no active loads on this lane right now, but the page updates automatically as new ones are posted.`,
      },
      {
        q: `How do I find a carrier or shipper on the ${o} to ${d} lane?`,
...
export default async function LanePage({ params }: Props) {
  ...
  const faqItems = t.faq(origin, destination, lane.loads.length);
  const faqJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
