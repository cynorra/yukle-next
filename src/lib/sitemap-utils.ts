...
// Workers Free plan's 10ms CPU-time cap and made this route intermittently
// fail/hang outright. Cut well below the byte ceiling to bound per-request
// CPU cost instead; the sitemap index computes ceil(total/PAGE_SIZE) fresh
// every time, so this just produces more, cheaper chunk files rather than
// fewer expensive ones — no crawler-facing change besides more page numbers.
